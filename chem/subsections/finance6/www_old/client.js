	...
	obj.socket.on('finance6',function(o){
		if(o && typeof o=='object'){
			...
			if(o['t']=='loginOk'){
				...
				obj.serviceWorker.checkDate(o['serviceWorkerDate']);
			};
			if(o['t']=='logout'){
				data.logout();
			};
...
obj.serviceWorker = function(){
	if ('serviceWorker' in navigator) {
		...
		caches.keys().then(keys => {
			let lastDate = 0;
			keys.forEach(key => {
				if ((/^chemshirov_/).test(key)){
					let date = key.replace(/^chemshirov_/, '').replace(/[^0-9]/g, '');
					if (date > lastDate)	lastDate = date;
				};
			});
			// let twoDaysAgo = ct2.date('',ct2.date('now') - 2*24*60*60*1000);
			// let twoDaysAgoDigits = twoDaysAgo.replace(/[^0-9]/g, '');
			// if (lastDate < twoDaysAgoDigits) {
				// navigator.serviceWorker.controller.postMessage({type: 'cacheFiles', base: data['base']});
			// };
		});
	};
};
obj.serviceWorker.checkDate = date => {
	if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
		navigator.serviceWorker.controller.postMessage({type: 'getCacheName', newDate: date});
	};
};
