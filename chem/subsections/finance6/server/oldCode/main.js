		...
		socket.on('finance6',function(o){
			...
			if((/\/finance6/).test(url)){
				...
				if(o && typeof o=='object'){
					...
					if(o['t']=='load2release')	obj.load2release(socket)
				};
			};
		});
...
obj.load2release = (socket) => {
	if (obj.o.common.containerName == 'chem-dev') {
		obj.load2release.copy('server/finance6/').then(() => {
			obj.load2release.copy('www/finance6/').then(() => {
				socket.emit('finance6', {t: 'load2release'})
			})
		})
	}
}
obj.load2release.copy = (path) => {
	return new Promise((r, e) => {
		obj.o.fs.readdir(obj.o.files + path, (err, files) => {
			if (files) {
				Object.keys(files).forEach(i => {
					let fileName = files[i]
					if ((/^[^\.]*\.(js|css|html)$/).test(fileName)) {
						obj.o.fs.copyFileSync(obj.o.files + path + fileName, obj.o.chem + 'https/' + path + fileName)
					}
				})
			}
			r()
		...
		obj.o.common.error('obj.load2release.copy', e)
	})
}
